...
sub_K3_std = df_K3_full.dropna(subset=[key])     # K3-PPUQ standalone 1-400
sub_K3_res = df_K3_resume.dropna(subset=[key])   # K3-PPUQ resume 350-400

# Splice K3-PPUQ trajectory: standalone 1-349 + resume 350-400 (best result at each phase)
sub_K3 = pd.concat([
    sub_K3_std[sub_K3_std["step"] < 350],
    sub_K3_res[sub_K3_res["step"] >= 350],
], ignore_index=True).sort_values("step").reset_index(drop=True)
...
ax.plot(sub_K3["step"], sub_K3[key] * 100, "-o",
        ...
        linewidth=2.0, markersize=4, alpha=0.95)
ax.set_xlabel("training step")
ax.set_ylabel("val_acc (GSM8K test, %)")
